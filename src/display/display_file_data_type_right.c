...
static const char RIGHT_CHAR_CHARDEV = 'T';
...
static void display_right(const mode_t mode, char filetype)
{
    for (int i = 0; i < 3; i++) {
        if (mode & USER_MASK[i])
    ...
    for (int i = 0; i < 3; i++) {
        if (filetype == 'c' && i == 2)
            my_putchar(RIGHT_CHAR_CHARDEV);
        else if (mode & EVERY_MASK[i])
            my_putchar(RIGHT_CHAR[i]);
        else
            my_putchar('-');
    }
...
void display_type_and_right(const file_t *file)
{
    ...
    display_right(file->mode, file->type);
    my_putchar(' ');
}
